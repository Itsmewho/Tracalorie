...
class CalorieTracker {
  ...
  _displayCaloriesPregress() {
    const progressEl = document.getElementById('calorie-progress');
    const precentge = (this._totalCalories / this._calorieLimit) * 100;
    const width = Math.min(precentge, 100);
...
class Storage {
  ...
  static removeMeal(id) {
    ...
    localStorage.setTiem('meals', JSON.stringify(meals));
  }
  ...
  static removeWorkout(id) {
    ...
    localStorage.setTiem('workouts', JSON.stringify(workouts));
  }
...
class App {
  ...
  _removeItem(type, e) {
    if (
      e.target.classList.contains('delele') ||
